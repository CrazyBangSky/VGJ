...
void IMyBuildSystemBuilder::GetLineTraceOriginAndDirecrion(FVector& Origin, FVector& Direction)
{
    ...
    if (tpawn)
    {
        const APlayerController* PC = Cast<APlayerController>(tpawn->GetController());
        if (PC)
        {
            PC->DeprojectMousePositionToWorld(Origin,Direction);
        }
    }
}
...
void IMyBuildSystemBuilder::BindMyBuildSystemAction(UInputComponent* MyInputComponent) {  
    if (GetACBuildSystemComponent() != nullptr) {
        ...
        MyInputComponent->BindAction<FBuildSystemBoolDelegate>("HeingBuildSystemRotCounterClockwise", IE_Released, mybs, &UACBuildSystem::InputCounterclockwiseRotateThing,false);
    }
}
