...
struct FBuildableInfo
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TSubclassOf<class AGamePlayNodeBase> actor_class;
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	float linetrace_distance = 40000.0f;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	class AGamePlayNodeBase* GhostBuilding = nullptr;
...
public:
	...
	UFUNCTION(BlueprintCallable)
		void InputRotateThing(bool bRotate);

	UFUNCTION(BlueprintCallable)
	void InputClockwiseRotateThing(bool bRotate);

	UFUNCTION(BlueprintCallable)
	void InputCounterclockwiseRotateThing(bool bRotate);
